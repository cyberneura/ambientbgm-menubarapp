#!/usr/bin/env bash
# Puts "dist/AmbientBGMMenubar.app" into dist/AmbientBGMMenubar_<version>_universal.dmg.
#
...
DMG="dist/AmbientBGMMenubar_${VERSION}_universal.dmg"
...
rm -f "$DMG"
hdiutil create -volname "AmbientBGM Menubar" -srcfolder "$STAGING" -ov -format UDZO "$DMG"
rm -rf "$STAGING"
